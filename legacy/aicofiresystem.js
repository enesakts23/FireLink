class ModernFireDashboard {
    constructor() {
        this.sensors = {
            ...
            pressure: {
                ...
                history: [],
                status: 'normal',
                trend: { direction: 'stable', value: '0.0 hPa' },
                color: '#6c757d',
                pattern: 'wave'
            },
            current: {
                ...
                history: [],
                status: 'normal',
                trend: { direction: 'stable', value: '0.0 A' },
                color: '#ffc107',
                pattern: 'sine'
            }
        };
        ...
        this.scenarios = {
            normal: {
                ...
                multipliers: { temperature: 1.0, humidity: 1.0, 'air-quality': 1.0, gas: 1.0, 'surface-temp': 1.0, 'surface-temp-2': 1.0, tvoc: 1.0, eco2: 1.0, no2: 1.0, co: 1.0, pressure: 1.0, current: 1.0 }
            },
            fire_simulation: {
                name: 'Fire Simulation',
                multipliers: { temperature: 1.8, humidity: 0.7, 'air-quality': 3.0, gas: 4.0, 'surface-temp': 2.0, 'surface-temp-2': 2.0, tvoc: 5.0, eco2: 3.5, no2: 6.0, co: 8.0, pressure: 0.95, current: 1.5 }
            },
            maintenance: {
                name: 'Maintenance Mode',
                multipliers: { temperature: 0.9, humidity: 1.1, 'air-quality': 0.8, gas: 0.6, 'surface-temp': 0.85, 'surface-temp-2': 0.85, tvoc: 0.9, eco2: 0.9, no2: 0.9, co: 0.9, pressure: 1.05, current: 0.8 }
            }
        };
    ...
    updateSystemStatus() {
        ...
        const boardHealthEl = document.getElementById('boardHealthValue');
        if (alertCountEl) {
        ...
        if (boardHealthEl) {
            boardHealthEl.textContent = `${this.systemState.boardHealth.toFixed(2)}%`;
        }
    ...
    getSensorIcon(sensorId) {
        const icons = {
            ...
            'surface-temp-2': 'fas fa-temperature-high',
            'tvoc': 'fas fa-atom',
            'eco2': 'fas fa-leaf',
            'no2': 'fas fa-cloud',
    ...
    getSensorType(sensorId) {
        const types = {
            ...
            'surface-temp-2': 'Infrared Sensor',
    ...
    updateWarningStatus(warning1, warning2) {
        const warningSensors = {
            warning1: ['temperature', 'humidity', 'gas', 'air-quality', 'no2', 'co', 'tvoc', 'eco2'],
            warning2: ['surface-temp', 'surface-temp-2', 'pressure', 'current']
        };

        // Process warning1
        warning1.split('').forEach((bit, index) => {
            if (bit === '1') {
                const sensorId = warningSensors.warning1[index];
                const statusElement = document.getElementById(`${sensorId}-status`);
                if (statusElement) {
        ...
        // Process warning2
        warning2.split('').forEach((bit, index) => {
            if (bit === '1') {
                const sensorId = warningSensors.warning2[index];
                const statusElement = document.getElementById(`${sensorId}-status`);
                if (statusElement) {
                    const statusBadge = statusElement.querySelector('.status-badge');
                    statusBadge.textContent = 'CRITICAL';
